...
export interface PrintRow {
  ...
  isSubItem?: boolean;
  isTotal?: boolean;
  status?: 'active' | 'inactive' | 'paid' | 'unpaid' | 'occupied' | 'vacant' | 'clear' | 'encumbered';
}
  ...
  if (property.units?.length) {
    ...
    property.units.forEach((unit) => {
      unitsData.push({
        ...
        value: formatNumber(Number(unit.unitArea)),
        additionalInfo: formatCurrency(Number(unit.rentAmount)),
        notes: unit.status,
...
function generatePrintHTML(printDataArray: PrintData[], title: string): string {
  ...
  // Determine if this is a detailed report for a single property to adjust the main title
  ...
  return `
    ...
    <head>
      ...
      <style>
        /* --- Page Setup with Professional Headers & Footers --- */
        @page {
          size: A4 landscape;
          /* Define generous margins to create space for header/footer */
          margin: 20mm 15mm 20mm 15mm; 
        }

        /* Using ::before and ::after on a container to create header/footer */
        .page-container::before {
            content: "${mainTitle}";
            position: fixed;
            top: -12mm; /* Position header within the top margin */
            left: 0;
            right: 0;
            width: 100%;
            text-align: center;
        ...
        .page-container::after {
            ...
            position: fixed;
            bottom: -15mm; /* Position footer within the bottom margin */
            left: 0;
            right: 0;
            width: 100%;
            text-align: center;
