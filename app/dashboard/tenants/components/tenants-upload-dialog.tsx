...
// CHANGE 1: Update the component's props to accept the callback function.
...
  tenantId,
  onDocumentAdded, // Destructure the new prop
...
  const [description, setDescription] = useState("");
  // The 'router' is no longer needed here since the parent handles the update.
...
    try {
      // CHANGE 2: Update the success callback in toast.promise
      await toast.promise(
        createDocument({
...
          tenantId,
          propertyId: '', 
          uploadedById: currentUserId?.id || '',
          description,
        }),
        {
          loading: "Saving document...",
          success: (newDocument) => { // `newDocument` is the data returned from `createDocument`
            // Call the parent component's handler function
            onDocumentAdded(newDocument);
            // We no longer need router.refresh()
            // router.refresh(); 
